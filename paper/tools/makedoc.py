...
def main(inp, outp):
    ...
    def number_sections(t):
        ...
        for L in t.split("\n"):
            m = re.match(r"^## (Code and data availability|Data availability statement|"
                         ...
                         r"Acknowledgements|Acknowledgments|References)\s*$", L)
            if m:
    ...
    def convert_refs(t):
        ...
        # so those cross-paper refs are left as literal text, not \ref'd to a same-numbered local sec.
        t = re.sub(r"((?:Paper\s+I{1,2}|companion(?:\s+paper|\s+note)?)[,;:)\s]{0,3})§",
                   lambda m: m[1] + "\x01", t)
