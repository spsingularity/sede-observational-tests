...
\begin{abstract}
We confront Structural Entropy Dark Energy (SEDE) --- a structure-gated,
H-linear horizon-entropy dark energy realized as a cubic
kinetic-gravity-braiding theory with braiding fraction b = 0.20 --- with
the full DESI DR2 BAO, Pantheon+ SN, and Planck 2018 (lowl TT + high-ℓ
TTTEEE-lite) likelihoods, computed through the mochi-class
...
\begin{keyword}
dark energy theory \sep cosmological parameters from CMBR \sep baryon acoustic
oscillations \sep modified gravity
\end{keyword}
...
(\texttt{expansion\_\allowbreak model\ =\ rho\_\allowbreak de}) and the perturbation sector as
the stable-parametrization \citep{Bellini:2014fua} Chebyshev
coefficients (\texttt{gravity\_\allowbreak model\ =\ stable\_\allowbreak params},
...
(\texttt{bao.\allowbreak desi\_\allowbreak dr2.\allowbreak desi\_\allowbreak bao\_\allowbreak all}); Pantheon+ SN
\citep{Brout:2022vxf} (\texttt{sn.\allowbreak pantheonplus}); Planck 2018 lowl TT
and high-ℓ \texttt{plik\_\allowbreak lite} TTTEEE \citep{Planck:2019nip} (clipy
...
\texttt{z\_\allowbreak gr\_\allowbreak smg\ =\ 99}, the SEDE lensed \(D_\ell ^\mathrm{TT}\) is
...
placed below the tachyonic window: \texttt{z\_\allowbreak gr\_\allowbreak smg\ =\ 5}, where
...
pipeline (\texttt{src/\allowbreak sims/\allowbreak sede\_\allowbreak fsigma8\_\allowbreak isw\_\allowbreak finitek.\allowbreak py}), 0.9236 from
the \(z_{\mathrm{gr}}\)-independent stage-2 cross-check, \(i.e. -7.6\)\%
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/solver_pathology_isw.png}}
...
\texttt{base\_\allowbreak plikHM\_\allowbreak TTTEEE\_\allowbreak lowl\_\allowbreak post\_\allowbreak BAO} covariance; spectral
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/sede_lcdm_mcmc_triangle.png}}
...
\texttt{planck\_\allowbreak 2018\_\allowbreak lensing.\allowbreak native} (CMB-marginalized). Without
...
\texttt{src/\allowbreak mcmc/\allowbreak phiphi\_\allowbreak joint\_\allowbreak refit.\allowbreak py}; earlier estimates and caveats
in \texttt{src/\allowbreak results/\allowbreak phiphi\_\allowbreak test\_\allowbreak result.\allowbreak md}).
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/fsigma8_prediction.png}}
...
\texttt{src/\allowbreak fsigma8\_\allowbreak prediction.\allowbreak json}, regenerated by
\texttt{src/\allowbreak sims/\allowbreak sede\_\allowbreak fsigma8\_\allowbreak isw\_\allowbreak finitek.\allowbreak py}.}\label{fig:3}
\end{figure}
...
\item
  ...
  \(\phi \phi\) likelihood (\texttt{planck\_\allowbreak 2018\_\allowbreak lensing.\allowbreak native},
...
\item
  ...
  here.
\item
  \textbf{Background-only H(z) cannot arbitrate, and this is structural
  rather than a data limitation.} Against the model-independent 31-point
  cosmic-chronometer compilation SEDE fits with
  \(\Delta \chi ^{2} = +0.04\) relative to flat \(\Lambda \mathrm{CDM}\)
  at equal parameter count, and the model's background shape is
  degenerate with an \((\Omega _{\mathrm{m}}\), \(H_{0}\))-shifted
  \(\Lambda \mathrm{CDM}\) at the \textasciitilde0.2\% level over
  \(z \in  [0\), 2{]} (an \(o\Lambda \mathrm{CDM}\) fit to the SEDE
  curve returns \(\Omega _{\mathrm{k}}\),eff = 0.000; script in the
  cosmology companion, \texttt{run\_\allowbreak deficit\_\allowbreak data.\allowbreak py}). No realistic
  expansion-history precision changes this --- which is a further reason
  the discriminating claims of this paper are placed in the growth
  \((f\sigma 8)\), tensor \((\Xi _{0})\), and \(\Delta\)-exponent
  sectors, where the gate's imprint is physical rather than
  reparameterizable.
\end{itemize}
...
repository (a tagged release is archived at Zenodo, DOI
10.5281/zenodo.21525526); the production chain files, the minima, and
the chain\(\to\)(\(\langle\chi^2\rangle\), \(p_{\mathrm{D}}\), DIC)
...
\texttt{rm\ python/\allowbreak classy.\allowbreak cpp\ \&\&\ pip\ install\ .\allowbreak \ -\/\allowbreak -no-build-isolation}).
The fork location is set via \texttt{MOCHI\_\allowbreak DIR}, the frozen SEDE inputs
are vendored in \texttt{src/\allowbreak mcmc/\allowbreak inputs/\allowbreak }, and likelihood data install
via \texttt{cobaya-install} from the included
\texttt{src/\allowbreak mcmc/\allowbreak likes\_\allowbreak install.\allowbreak yaml} (full setup in
\texttt{src/\allowbreak README.\allowbreak md}). The growth/ISW predictions \((\sigma 8\),
\(f\sigma 8\), the \(C_\ell ^\mathrm{TT}(2)=0.924\) ISW ratio)
regenerate directly from
\texttt{src/\allowbreak sims/\allowbreak sede\_\allowbreak fsigma8\_\allowbreak isw\_\allowbreak finitek.\allowbreak py}; the production chains
were run with the exact \texttt{src/\allowbreak mcmc/\allowbreak lcdm\_\allowbreak mc.\allowbreak yaml} /
\texttt{src/\allowbreak mcmc/\allowbreak sede\_\allowbreak mc.\allowbreak yaml} released here (Rminus1 stop 0.02; the
loose \(Rminus1_{\mathrm{cl},\mathrm{stop}}\) 0.2 on the tails should
be tightened for the released run). The Planck-lensing \((\phi \phi )\)
test of §\ref{sec-5}: the definitive BAO/SN/BBN-anchored joint refit is
\texttt{src/\allowbreak mcmc/\allowbreak phiphi\_\allowbreak joint\_\allowbreak refit.\allowbreak py} (needs the full DESI DR2 BAO +
...
(\texttt{src/\allowbreak mcmc/\allowbreak phiphi\_\allowbreak As.\allowbreak py},
\texttt{src/\allowbreak mcmc/\allowbreak phiphi\_\allowbreak fullrefit.\allowbreak py}) and all caveats are in
\texttt{src/\allowbreak results/\allowbreak phiphi\_\allowbreak test\_\allowbreak result.\allowbreak md}.
...
with the assistance of Claude Opus 4.x (Anthropic). All numerical claims
trace to the released, rerunnable configurations and scripts. No AI tool
is an author.
