#!/usr/bin/env bash
# Build the Physics of the Dark Universe-class PDF (jcappub.sty) for Paper IV from SEDE_observational_tests.md.
...
perl -0pi -e 's#(\\begin\{keyword\}\s*\n)([^\\]*?)(\n\s*\\end\{keyword\})#my ($a,$k,$z)=($1,$2,$3); $k =~ s{[;,]\s*}{ \\sep }g; "$a$k$z"#se' tex/SEDE_observational_tests.tex
# make long monospace paths breakable (prevents right-margin overflow)
perl -0pi -e 's{\\texttt\{([^{}]*)\}}{"\\texttt{".($1=~s#([/._])#$1\\allowbreak #gr)."}"}ge' tex/SEDE_observational_tests.tex
